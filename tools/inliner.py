...
def inline_module(file, package, packagePath, text, processedModules, copyMdnvlib):
    with open(file, 'r', encoding='utf-8') as f:
        print(f'Processing "{file}"...')
        if copyMdnvlib:
            target = file.replace('/../mdnovel', '')
                ...
                if 'import ' in line:
                    ...
                    if (importModule is not None) and (package in importModule.group(1)):
                        ...
                        if not (moduleName in processedModules):
                            ...
                            text = inline_module(
                                f'{moduleName}.py', package, packagePath, text, processedModules, copyMdnvlib)
...
def run(sourceFile, targetFile, package, packagePath, copymdnvlib=False):
    text = ''
    processedModules = []
    text = inline_module(sourceFile, package, packagePath, text, processedModules, copymdnvlib)
